...
import { resetValuesMap } from 'presentation/modals/adverts/CreateAdvertModal';
import url from 'utils/url';

export interface AdvertEditModalProps extends DialogOptions {
    id: number;
}

export default function UpdateAdvertModal({ id, onClose }: AdvertEditModalProps): ReactElement {
    const updateAdvertUseCase = Container.resolve(UpdateAdvertUseCase);
    const getAdvertUseCase = Container.resolve(GetAdvertUseCase);

    async function fetchData(): Promise<AdvertModel | null> {
        return getAdvertUseCase.handle(id);
    }

    const { data: advert } = useQuery(fetchData, [GetAdvertUseCase.name, id]);

    const [previewImage, setPreviewImage] = useState<FileShape | null>(advert?.image || null);
    ...
    const {
        ...
        setValue,
        setValues,
    } = useForm<AdvertModel>({ defaultValues: { ...advert } });

    const { linkType } = useWatch({ control });
    ...
    async function onSubmit(values: AdvertModel): Promise<boolean> {
        ...
        const result = await updateAdvertUseCase.handle(values);
                        ...
                        <Column>
                            ...
                            {linkType === 'article' && (
                                <PublicationArticleSelect
                                    ref={publicationRef}
                                    initialArticleId={advert?.linkedArticleId}
                                    initialPage={advert?.linkedPage}
                                    onChange={(linkage) => {
                                        setValues({
