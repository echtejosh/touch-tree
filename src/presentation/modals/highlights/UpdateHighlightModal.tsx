...
export interface HighlightEditModalProps extends DialogOptions {
    id: number;
}

export default function UpdateHighlightModal({
    id,
    onClose,
}: HighlightEditModalProps): ReactElement {
    const getHighlightUseCase = Container.resolve(GetHighlightUseCase);
    const getPublicationPageImageUseCase = Container.resolve(GetPublicationPageUseCase);
    const updateHighlightUseCase = Container.resolve(UpdateHighlightUseCase);

    const { data: highlight } = useQuery(() => getHighlightUseCase.handle(id), [GetHighlightUseCase.name, id]);
    ...
    const {
        ...
        handleSubmit,
        setValues,
    } = useForm<HighlightModel>({ defaultValues: { ...highlight, customImage: null } });

    const formData = useWatch({ control });

    async function fetchPublicationPage(): Promise<NewsstandPublicationPageModel | null> {
        return getPublicationPageImageUseCase.handle({
    ...
    const { data: publicationPage, isLoading: isPageImageLoading } = useQuery(
        ...
        [GetPublicationPageUseCase.name, id, formData?.linkedArticleId, formData?.linkedPage],
    );
    ...
    const getImageSource = useMemo(() => {
        ...
        const isDifferentLink = highlight?.linkedArticleId !== formData?.linkedArticleId
            || highlight?.linkedPage !== formData?.linkedPage;

        return isDifferentLink ? publicationPage?.imageData : highlight?.image?.content;
    }, [
        highlight?.linkedArticleId,
        highlight?.linkedPage,
    ...
    async function onSubmit(values: HighlightModel): Promise<boolean> {
        ...
        const result = await updateHighlightUseCase.handle(values);

        if (result) {
            onClose();
            useInvalidateQuery([GetHighlightsUseCase.name]);
            useInvalidateQuery([GetHighlightUseCase.name, id]);
        }
