...
import PublicationInteractionImagePreview from 'presentation/components/PublicationInteractionImagePreview';
import GetPublicationInteractionsUseCase from 'application/usecases/publications/GetPublicationInteractionsUseCase';
import { CheckboxElement, SelectElement, useWatch } from 'react-hook-form-mui';
import { PublicationInteraction } from 'domain/models/PublicationInteraction';
import CreatePublicationInteractionUseCase from 'application/usecases/publications/CreatePublicationInteractionUseCase';
...
export interface CreatePublicationInteractionDialogProps extends DialogOptions {
    pageId: number | null;
    page: number | null;
 ...
 * @constructor
 */
export default function CreatePublicationInteractionDialog({
    publicationId,
    page,
    pageId,
    onClose,
}: CreatePublicationInteractionDialogProps): ReactElement {
    const getPublicationInteractionsUseCase = Container.resolve(GetPublicationInteractionsUseCase);
    const createPublicationInteractionUseCase = Container.resolve(CreatePublicationInteractionUseCase);
    ...
    const {
        ...
        defaultValues: {
            ...
            // effectId: publicationInteractions?.effects?.[0].id || undefined,
        },
    });
    ...
    async function onSubmit(values: PublicationInteraction) {
        ...
        const result = await createPublicationInteractionUseCase.handle(payload);

        if (result) {
            useInvalidateQuery([GetPublicationInteractionUseCase.name, result]);
        ...
        <Box>
            <Column columns={2} gap={0} height={600}>
                ...
                <Column gap={4} p={4}>
                    ...
                    {!formData.percentageRight && (
                        <Alert
                            severity='warning'
                            sx={{ border: 'none' }}
                        >
                            No area selected. Please click and drag on the image to select an area
                        </Alert>
                    )}
                    ...
                    <Column flex={1} justifyContent='space-between'>
                        ...
                        <Row gap={2} justifyContent='space-between'>
                            ...
                            <SubmitButton
                                disabled={!formData.percentageRight}
