import React from 'react';
import {
    Button, Typography,
} from '@mui/material';
import Container from 'infrastructure/services/Container';
import { useForm, useInvalidateQuery } from 'presentation/hooks';
import { DialogOptions } from 'presentation/providers/DialogProvider';
import GetRelationAdvertsUseCase from 'application/usecases/relation-adverts/GetRelationAdvertsUseCase';
import RemoveRelationAdvertUseCase from 'application/usecases/relation-adverts/RemoveRelationAdvertUseCase';
import { Box, Column, Row } from 'presentation/components/layout';
import { SubmitButton } from 'presentation/components';
import { themePalette } from 'presentation/theme';

interface DeleteRelationAdvertsModalProps extends DialogOptions {
    ids: number[];
}

export default function RemoveRelationAdvertsModal({
    ids,
    onClose,
}: DeleteRelationAdvertsModalProps) {
    const removeRelationAdvertUseCase = Container.resolve(RemoveRelationAdvertUseCase);

    const {
        handleSubmit,
    } = useForm();

    async function onSubmit(): Promise<boolean> {
        const deletionPromises = ids.map((id) => removeRelationAdvertUseCase.handle(id));

        const results = await Promise.all(deletionPromises);
        const allSucceeded = results.every(Boolean);

        if (allSucceeded) {
            useInvalidateQuery([GetRelationAdvertsUseCase.name]);
            onClose();
        }

        return allSucceeded;
    }

    return (
        <Box m={4}>
            <Typography
                sx={{
                    fontSize: 24,
                    fontWeight: 600,
                    mb: 3,
                }}
            >
                Delete selected advert(s)?
            </Typography>

            <Column mb={4}>
                <Typography variant='subtitle1'>
                    Are you sure you want to delete selected advert(s)? This action cannot be undone.
                </Typography>
            </Column>

            <Row justifyContent='space-between'>
                <Button
                    color='primary'
                    onClick={onClose}
                    size='large'
                    variant='outlined'
                >
                    Cancel
                </Button>

                <SubmitButton
                    handleSubmit={handleSubmit}
                    onSubmit={onSubmit}
                    size='large'
                    sx={{
                        background: themePalette.error.main,
                        ':hover': {
                            background: themePalette.error.light,
                        },
                    }}
                    variant='contained'
                >
                    Delete
                </SubmitButton>
            </Row>
        </Box>
    );
}
