...
export default function PlatformMiscDropdown({ open }: { open: boolean }) {
    ...
    return (
        <EditorSidebarDropdown label='Misc.' open={open}>
            <ColorSelect
                control={control}
                defaultColor={editorColorsPalette.separator}
                label='Separator'
                name='separatorColor'
                onBlur={(e) => setInteraction({ name: 'separatorColor', value: e.target.value })}
            />
